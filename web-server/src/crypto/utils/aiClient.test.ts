...
const mock = vi.hoisted(() => ({ generate: vi.fn(), model: vi.fn() }));
vi.mock("@google/generative-ai", () => ({
  GoogleGenerativeAI: class {
    getGenerativeModel(config: unknown) {
      mock.model(config);
      return { generateContent: mock.generate };
    }
  },
}));
...
describe("AI evidence envelope", () => {
  it("retains grounding, model and finish reason without changing text callers", async () => {
    ...
    mock.generate.mockResolvedValue({
      response: { text: () => "answer", candidates: [{ groundingMetadata, finishReason: "STOP" }] },
    });
    ...
    expect(mock.model).toHaveBeenCalledWith(expect.objectContaining({ tools: [{ googleSearch: {} }] }));
    expect(await client.generateCompletion("test")).toBe("answer");
  });
  it("does not fabricate missing grounding", async () => {
    mock.generate.mockResolvedValue({ response: { text: () => "answer", candidates: [{ finishReason: "STOP" }] } });
