...
# Database
MONGODB_URI=mongodb://localhost:27017/spikey-coins-dev
...
pm2 start dist/server.js --name "spikey-coins-backend" || pm2 restart "spikey-coins-backend"
pm2 save
...
echo "2. Restart aftyer edit: pm2 restart spikey-coins-backend"
echo "3. Check logs: pm2 logs spikey-coins-backend"
