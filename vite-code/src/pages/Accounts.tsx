  ...
  const handleAuthAccount = async (accountId: string) => {
    try {
      ...
      switch (account.accountType) {
        case 'upstox':
          authEndpoint = account.metadata?.sandbox ? '/api/auth/upstox/sandbox-token' : '/api/auth/upstox/login';
          break;
        case 'binance':
      ...
      if (account.accountType === 'upstox' && account.metadata?.sandbox && authEndpoint.includes('sandbox-token')) {
  ...
  return (
    <PageLayout title="Account Management">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontSize: '2.5rem', fontWeight: 700, margin: '0 0 8px 0' }}>Trading Accounts</h1>
      ...
      {error && (
        <div style={{ background: '#fff3cd', border: '1px solid #ffeaa7', borderRadius: '8px', padding: '16px', marginBottom: '24px', color: '#856404' }}>
          <p>⚠️ {error}</p>
        </div>
      )}
      ...
      {accounts.length === 0 ? (
        <EnhancedCard>
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 600, margin: '0 0 12px 0' }}>No Trading Accounts</h3>
          ...
          <EnhancedCard title="Account Overview">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '20px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: '#2196f3', marginBottom: '4px' }}>
                  {accounts.length}
                </div>
                <div style={{ fontSize: '0.9rem', color: '#666', fontWeight: 500 }}>Total Accounts</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: '#2196f3', marginBottom: '4px' }}>
                  {accounts.filter(acc => acc.accessToken).length}
                </div>
                <div style={{ fontSize: '0.9rem', color: '#666', fontWeight: 500 }}>Connected</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: '#2196f3', marginBottom: '4px' }}>
                  {accounts.filter(acc => acc.accountType === 'upstox').length}
                </div>
                <div style={{ fontSize: '0.9rem', color: '#666', fontWeight: 500 }}>Upstox</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: '#2196f3', marginBottom: '4px' }}>
                  {accounts.filter(acc => acc.accountType === 'kite').length}
                </div>
                <div style={{ fontSize: '0.9rem', color: '#666', fontWeight: 500 }}>Kite</div>
              </div>
            </div>
          </EnhancedCard>
        </div>
