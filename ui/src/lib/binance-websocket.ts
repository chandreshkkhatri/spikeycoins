...
class BinanceWebSocketService {
  ...
  private handleMessage(data: string) {
    try {
      ...
      if (Array.isArray(message)) {
        message.forEach((tickerData: Record<string, unknown>) => {
          const symbol = tickerData.s as string;
          if (!symbol) return;
   ...
   */
  private formatTickerData(rawData: Record<string, unknown>): BinanceTickerData {
    return {
      symbol: rawData.s as string,
      lastPrice: (rawData.c as string) || (rawData.p as string),
      priceChange: (rawData.p as string) || "0",
      priceChangePercent: (rawData.P as string) || "0",
      high: (rawData.h as string) || "0",
      low: (rawData.l as string) || "0",
      volume: (rawData.v as string) || "0",
      quoteVolume: rawData.q as string | undefined,
      openTime: rawData.O as number | undefined,
      closeTime: rawData.C as number | undefined,
    };
  }
...
declare global {
  ...
  var __BINANCE_WS__: BinanceWebSocketService | undefined;
}

// SSR-safe singleton creation
const createInstance = (): BinanceWebSocketService => {
  if (typeof window === "undefined") {
    return new BinanceWebSocketService();
  }

  const globalAny = globalThis as typeof globalThis & { __BINANCE_WS__?: BinanceWebSocketService };
  const existing = globalAny.__BINANCE_WS__;
  const instance = existing ?? new BinanceWebSocketService();
  globalAny.__BINANCE_WS__ = instance;
  return instance;
};

const binanceWebSocketService = createInstance();
