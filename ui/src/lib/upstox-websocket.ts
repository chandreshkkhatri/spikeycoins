...
export class UpstoxWebSocket implements WebSocketManager {
    ...
    try {
      ...
      if (instrumentKeys.length) {
        ...
        if (json?.success && json?.data) {
          const entries = Object.entries(json.data) as [string, Record<string, unknown>][];
          for (const [ik, feed] of entries) {
            ...
            const feedLtpc = feed?.ltpc as { ltp?: number; cp?: number } | undefined;
            const feedOhlc = feed?.ohlc as { open?: number; high?: number; low?: number; close?: number } | undefined;
            const feedBidAsk = feed?.bidAsk as { bid?: number; ask?: number; bidQty?: number; askQty?: number } | undefined;
            if (feedLtpc) {
              const ltp = feedLtpc.ltp ?? upd.lastPrice;
              const cp = feedLtpc.cp ?? upd.close;
              upd.lastPrice = ltp;
              upd.close = cp;
              if (cp && cp > 0 && ltp) {
            ...
            if (feedOhlc) {
              upd.open = feedOhlc.open ?? upd.open;
              upd.high24h = feedOhlc.high ?? upd.high24h;
              upd.low24h = feedOhlc.low ?? upd.low24h;
              upd.close = feedOhlc.close ?? upd.close;
            }
            if (feedBidAsk) {
              upd.bid = feedBidAsk.bid ?? upd.bid;
              upd.ask = feedBidAsk.ask ?? upd.ask;
              upd.bidQty = feedBidAsk.bidQty ?? upd.bidQty;
              upd.askQty = feedBidAsk.askQty ?? upd.askQty;
            }
  ...
  private async handleBinaryMessage(buffer: ArrayBuffer): Promise<void> {
    try {
      ...
      const decoded = this.FeedResponseType.decode(bytes) as protobuf.Message & {
        type?: number;
      ...
      for (const instrumentKey of Object.keys(feeds)) {
        const feed = feeds[instrumentKey] as {
          ltpc?: { ltp?: number; cp?: number };
          fullFeed?: {
            marketFF?: { ltpc?: { ltp?: number; cp?: number }; marketOHLC?: { ohlc?: Array<{ open?: number; high?: number; low?: number; close?: number; vol?: number }> } };
            indexFF?: { ltpc?: { ltp?: number; cp?: number }; marketOHLC?: { ohlc?: Array<{ open?: number; high?: number; low?: number; close?: number; vol?: number }> } };
          };
          firstLevelWithGreeks?: {
            ltpc?: { ltp?: number; cp?: number };
            firstDepth?: { bidQ?: number; bidP?: number; askQ?: number; askP?: number };
          };
        };
        // Try original key first (for proper casing like "Nifty 50"), then uppercase
        const symbol =
...
// SSR-safe singleton creation
const createInstance = (): UpstoxWebSocket => {
  if (typeof window === "undefined") {
    return new UpstoxWebSocket();
  }

  const globalAny = globalThis as typeof globalThis & { __UPSTOX_WS__?: UpstoxWebSocket };
  const existing = globalAny.__UPSTOX_WS__;
  const instance = existing ?? new UpstoxWebSocket();
  globalAny.__UPSTOX_WS__ = instance;
  return instance;
};

export const upstoxWebSocket = createInstance();
