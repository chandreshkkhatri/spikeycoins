...
| **BG-024** | Tech Debt | Extract a `useTradingWindow` hook from the 2800-line `TradingWindow.tsx` (33 `useState`/11 `useEffect`/31 handlers), mirroring `useWatchlist`. Heavy, order-entry path — blocked on BG-028's test coverage landing first. Sprint-2, iteration 3. | `ui` / `components` | 🟥 To Do |
